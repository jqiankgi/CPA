...
# Jifeng revised version, add function calDeltaHS and calDeltaG, changed function Tm_staluc(s,dnac=250,saltc=50,rna=0), updated on Feb 15th.
...
def Tm_staluc(s,dnac=250,saltc=50,rna=0):
    ...
    if rna==0:
        ...
        *36.9 + (overcount(sup,"GG"))*32.7 + (overcount(sup,"CC"))*29.7
        ...
        raise ValueError("rna = %r not supported" %rna)
    C_Mg = 3.0 # mM 
    C_MonovalentIon = 50.0 #mM 
    C_dNTP = 0.00008 #mM 
    C_SodiumEquivalent = C_MonovalentIon + 120 * math.sqrt(C_Mg-C_dNTP)
    ds = ds-0.368*(len(s)-1)*math.log(C_SodiumEquivalent/1e3)
...
def calDeltaG(seq1, seq2, mono_conc=50, Mg_conc=3, dntp_conc=0.00008, deltaH=None, deltaS=None): 
    """ Calculate the free Gibbs energy """

    mono_conc = float(mono_conc)
    Mg_conc = float(Mg_conc)
    dntp_conc = float(dntp_conc)

    if not (deltaH and deltaS):
	deltaH, deltaS = calDeltaHS(seq1, seq2)

    # Calculate the free Gibbs energy
    tao = 273.15 + 37 # Constant temperature tao in Kelvin

    mono_conc = mono_conc + 120 * math.sqrt(Mg_conc-dntp_conc)
    mono_conc = mono_conc / 1000

    deltaS_adjust = deltaS + 0.368 * (len(seq2) - 1) * math.log(mono_conc, math.e)
